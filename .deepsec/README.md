...
Currently configured project: `membrane` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/membrane/SETUP.md` to fill in
   `data/membrane/INFO.md`.
...
`--project-id membrane` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/membrane/`.
