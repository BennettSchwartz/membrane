# Agent setup for `membrane`

This is a deepsec scanning workspace. Project `membrane` was just registered
(target: `..`). Setup is incomplete — `data/membrane/INFO.md`
...
2. **Fill in `data/membrane/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id membrane
pnpm deepsec process --project-id membrane
```
