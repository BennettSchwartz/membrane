...
<Steps>
  ...
  <Step title="Create or reinforce">
    If no competence record exists for this skill name in the derived scope, a new one is created with `SuccessCount` set to the number of matching episodes and `SuccessRate = 1.0`. If a record already exists in that same scope, only newly observed source episodes reinforce it, append provenance, add `derived_from` links, and increment `SuccessCount`. Matching by both skill name and scope keeps project-specific procedures from reinforcing unrelated projects that happen to use the same tool sequence.
  </Step>
</Steps>
...
The extracted competence record inherits the maximum sensitivity of its source episodes. Patterns are grouped separately within each scope, and each scope must independently meet the occurrence threshold. Source and destination policy are rechecked inside the write transaction; a source that moved scopes is deferred to a later run. Consolidated provenance links back to each contributing episode via `derived_from` relations. Reinforcement is source-idempotent: already-recorded episodes do not increase salience or performance counts. If their sensitivity has increased, the derived record is promoted and the policy change is audited.
...
1. **Applicability** — the record's `Confidence` field, or a vector similarity score when embeddings are configured.
...
When an embedding endpoint is configured, the selector can replace the `record.Confidence` proxy with actual embedding similarity stored in Postgres + pgvector:
...
Embeddings are stored for all embeddable memory records, including entity, semantic, competence, and plan graph records, when `embedding_endpoint` and `embedding_model` are configured. Configure them together in `config.yaml`:

Selector similarity only replaces confidence when both the query vector and the stored record vector are usable: same dimension, finite values, and at least one non-zero component. Empty, all-zero, non-finite, missing, or mismatched vectors are treated as unavailable so a corrupt embedding cannot become a high-confidence applicability signal.
