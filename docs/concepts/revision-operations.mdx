...
Atomically replaces an old record with a new version. The old record is retracted (salience set to 0, semantic status set to `retracted`). The new record is linked to the old via a `supersedes` relation and provenance source; the old record receives the inverse `superseded_by` relation.
...
4. Old record gets a `superseded_by` relation pointing to the replacement.
5. New record's provenance includes the old record as a source.
6. Audit entry (`revise`) is added to the old record; audit entry (`create`) is added to the new record.
7. Both updates are committed in a single transaction.
...
Creates a new record derived from an existing source record. Unlike Supersede, both the source and the forked record remain active. Fork is used for conditional variants — facts that are true under specific conditions (`ValidityModeConditional`). For semantic payloads, the forked record's revision status is set to `active`.
...
3. Source record gets a `derived_semantic` relation pointing to the fork.
4. Audit entry (`fork`) is added to the source record; audit entry (`create`) is added to the forked record.
5. Both records remain active with their original salience.
...
Marks a record as contested, indicating that conflicting evidence exists. The record remains visible but its semantic status is set to `contested`. A `contested_by` relation is added pointing to the conflicting record, and that record receives the inverse `contests` relation.
...
4. A `contests` relation is added from `contestingRef` back to the target.
5. Audit entry (`revise`) is added to the target record.
...
Combines multiple unique source records into a single consolidated record. All source records are retracted, the merged record receives `derived_from` relations to each source, and each source receives a `derived_semantic` relation to the merged record. For semantic payloads, the merged record's revision status is set to `active`.
...
4. Each source gets a `derived_semantic` relation to the merged record.
5. Audit entry (`merge`) is added to each source; audit entry (`create`) is added to the merged record.
6. All changes are committed in a single transaction.
...
|-----------|------------|---------------|------------------|
| Supersede | Retracted | `active` | `supersedes` (new -> old), `superseded_by` (old -> new) |
| Fork | Stays active | `active` | `derived_from` (fork -> source), `derived_semantic` (source -> fork) |
| Contest | Status → `contested` | — | `contested_by` (target -> evidence), `contests` (evidence -> target) |
| Retract | Retracted | — | none |
| Merge | All retracted | `active` | `derived_from` (merged -> each source), `derived_semantic` (source -> merged) |
