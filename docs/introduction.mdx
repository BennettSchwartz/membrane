...
<CardGroup cols={2}>
  ...
  <Card title="Vector-aware retrieval" icon="chart-scatter">
    Postgres + pgvector is the required runtime store; when embeddings are configured, all record types can use hybrid vector+salience ranking.
  </Card>
  ...
  <Card title="Security and operations" icon="lock">
    Postgres-backed persistence, optional TLS, API key authentication, configurable rate limiting, full audit logs.
  </Card>
</CardGroup>

## Deployment tiers

Membrane is Postgres + pgvector first. Embedding and LLM services add ranking and extraction quality on top of the same required database substrate:

| Tier | Store | Embedding | LLM | Behavior |
|------|-------|-----------|-----|----------|
| 1 | Postgres + pgvector | — | — | Typed records, graph edges, trust gating, decay, and revision history |
| 2 | Postgres + pgvector | Yes | — | Hybrid vector+salience ranking for all record types |
| 3 | Postgres + pgvector | Yes | Yes | Full system with LLM-backed episodic to semantic extraction |
...
<CardGroup cols={2}>
  ...
  <Card title="Memory types" icon="layers" href="/concepts/memory-types">
    Learn the five memory layers, entity graph, and lifecycle rules.
  </Card>
