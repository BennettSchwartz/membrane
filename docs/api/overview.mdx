...
| TLS | Optional on loopback; required for API-key-authenticated non-loopback listeners unless explicitly overridden |
...
The server rejects API-key authentication over plaintext on non-loopback
listeners by default. `allow_insecure_credentials: true` is an explicit escape
hatch for trusted development networks; production deployments should use TLS.
