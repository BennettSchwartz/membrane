...
The `retrieval` package implements layered memory retrieval per RFC 15.8. Records are queried in canonical layer order (`working → semantic → competence → plan_graph → episodic`), filtered by trust context and salience. With Postgres + pgvector configured (recommended), results are ranked using hybrid vector+salience scoring. Competence and plan-graph candidates go through an additional multi-solution selector. Without embeddings, results fall back to salience-only ranking.
...
`Membrane.New` wires the correct constructor automatically based on the `Config`. With Postgres + pgvector + an embedding endpoint configured, it uses `NewServiceWithVectorRanker` for hybrid vector+salience ranking across all record types.
...
When a `VectorRanker` is configured, `Retrieve` ranks all record types using a hybrid score: 70% vector similarity, 30% salience. This replaces the salience-only fallback for semantic, episodic, and working records.
...
<ParamField path="TaskDescriptor" type="string">
  Natural-language description of the current task. When an embedding service is configured, this is encoded into a query vector used for hybrid ranking and applicability scoring.
</ParamField>

<ParamField path="QueryEmbedding" type="[]float32">
  Optional pre-computed embedding for the query. When set, Retrieve uses this instead of calling `EmbedQuery`, avoiding an extra API round-trip. Useful for batching or avoiding rate limits.
</ParamField>
