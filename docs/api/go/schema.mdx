...
<ParamField path="Type" type="MemoryType" required>
  Memory category. One of `episodic`, `working`, `entity`, `semantic`, `competence`, or `plan_graph`.
</ParamField>
...
<ParamField path="Payload" type="Payload" required>
  Type-specific structured content. One of `EpisodicPayload`, `WorkingPayload`, `EntityPayload`, `SemanticPayload`, `CompetencePayload`, or `PlanGraphPayload`.
</ParamField>
...
`ParseEntityIdentifierTokens` extracts explicit `namespace:value` entity identifiers from descriptors and mention text while skipping ordinary URLs, email addresses, and clock times. Retrieval and capture resolution both use this parser, so a token such as `github:BennettSchwartz/orchid` resolves against the same entity identifier index in both paths.
...
<ParamField path="Predicate" type="string" required>
  Relationship type. Predicates are normalized to lower snake case before storage and must not be empty. Common values: `supports`, `contradicts`, `derived_from`, `supersedes`, `contested_by`.
</ParamField>
...
<ParamField path="Weight" type="float64">
  Strength of the relationship. Must be finite and in range `[0, 1]`; omitted/zero weights are stored as the default edge strength.
</ParamField>
...
The schema package exports constants for Membrane-generated graph predicates so ingestion, retrieval, revision, SDKs, and tests do not drift on raw strings:

```go
schema.GraphPredicateMentionsEntity
schema.GraphPredicateMentionedIn
schema.GraphPredicateDerivedSemantic
schema.GraphPredicateDerivedFrom
schema.GraphPredicateSubjectEntity
schema.GraphPredicateFactSubjectOf
schema.GraphPredicateObjectEntity
schema.GraphPredicateFactObjectOf
schema.GraphPredicateReferencesRecord
schema.GraphPredicateReferencedBy
...
Use `schema.NormalizeGraphPredicate(predicate)` for graph predicates from external
or model-generated input. It lowercases input, splits camelCase, and treats
punctuation, whitespace, hyphens, and underscores as separators, so
`"Depends On"`, `"depends-on"`, `"depends_on"`, and `"dependsOn"` store the same
predicate. Postgres storage and the internal test store apply this normalization
again at relation write boundaries, including `Create`, `Update`, and
`AddRelation`.

Use `schema.InverseGraphPredicate(predicate)` when materializing reverse edges. It
normalizes input before matching. Unknown predicates return
`inverse_of_<normalized_predicate>`, matching capture-time relation candidate
behavior.
...
| `entity` | `MemoryTypeEntity` | Canonical graph spine: projects, files, services, people, tools, and aliases that connect other memory records. |
...
Semantic payloads require non-empty `Subject` and canonicalizable `Predicate` fields, a non-nil `Object`, and a `Validity.Mode` of `global`, `conditional`, or `timeboxed`. Semantic predicates normalize to lower snake case before storage and exact-fact matching. `MemoryRecord.Validate` rejects malformed direct semantic records before storage.

Entity payloads require a non-empty `CanonicalName`. Any supplied `EntityIdentifier` must also have a non-empty `Namespace` and `Value`, so direct entity records cannot silently skip identifier indexing. `MemoryRecord.Validate` rejects malformed direct entity records before storage.
