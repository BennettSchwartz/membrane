...
```bash yarn
yarn add @bennettschwartz/membrane
```

```bash pnpm
pnpm add @bennettschwartz/membrane
```
...
The package exports the following from the top-level `@bennettschwartz/membrane` entry point:
...
```typescript
import { OutcomeStatus } from "@bennettschwartz/membrane";
...
```typescript
import { TaskState } from "@bennettschwartz/membrane";
...
```typescript
import { MembraneClient, MembraneError } from "@bennettschwartz/membrane";
