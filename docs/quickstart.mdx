...
- **Go 1.27.1** is the preferred build toolchain, selected automatically by `go.mod`; the Go library supports **Go 1.24+**
- **Make** to run build and test targets
- **Docker Compose** for the local Postgres + pgvector quickstart database
- **protoc >= 3.20** only if you plan to regenerate gRPC stubs
- **Node.js 20.19+** for the TypeScript SDK
  ...
  <Tab title="Go library (embedded)">
    ...
    <Steps>
      ...
      <Step title="Start Postgres + pgvector">
        ...
        The sample below connects to the repository's local `pgvector/pgvector:pg16` service.
      </Step>
      ...
      <Step title="Run it">
        ...
        Membrane connects to the configured Postgres database and ensures the pgvector-backed schema exists on startup. `m.Start(ctx)` starts background decay, consolidation, and one startup embedding backfill when embeddings are configured; individual backfill failures are logged as an aggregate error so incomplete vector coverage is visible. `m.Stop()` cancels that background context, waits for jobs to finish, and then closes Postgres.
      </Step>
    </Steps>
  </Tab>
  <Tab title="Daemon (gRPC server)">
    ...
    <Steps>
      ...
      <Step title="Verify startup">
        The daemon logs `membraned: listening on 127.0.0.1:9090` when it starts successfully. Send `SIGINT` or `SIGTERM` to trigger graceful shutdown.
      </Step>
    </Steps>
