...
func (s *Service) CaptureMemory(ctx context.Context, req CaptureMemoryRequest) (*CaptureMemoryResponse, error) {
	...
	if s.interpreter != nil {
		interpreted, err := s.interpreter.Interpret(ctx, InterpretRequest{
			Source:           req.Source,
	...
	candidates, err := s.fetchCaptureCandidates(ctx, req, sourceRecord.Interpretation)
	if err != nil {
		return nil, fmt.Errorf("ingestion: fetch candidates: %w", err)
	}
	...
	for idx := range sourceRecord.Interpretation.Mentions {
		...
		entity, created, entityEdges, err := s.resolveMention(ctx, sourceRecord, mention, req.Scope, req.Source, ts, candidates)
		if err != nil {
			return nil, err
		}
		if entity == nil {
	...
	for idx := range sourceRecord.Interpretation.ReferenceCandidates {
		...
		target, ok := resolveReferenceCandidate(ref.Ref, candidates)
	...
	sourceRecord.Interpretation.Status = schema.InterpretationStatusResolved
	if err := s.store.Update(ctx, sourceRecord); err != nil {
...
func (s *Service) resolveMention(ctx context.Context, source *schema.MemoryRecord, mention *schema.Mention, scope, actor string, ts time.Time, candidates []*schema.MemoryRecord) (*schema.MemoryRecord, *schema.MemoryRecord, []schema.GraphEdge, error) {
	entity := findMatchingEntity(mention, candidates)
	var created *schema.MemoryRecord
	if entity == nil {
		entity = buildEntityRecord(mention, scope, actor, ts)
...
func buildEntityRecord(mention *schema.Mention, scope, actor string, ts time.Time) *schema.MemoryRecord {
	canonicalName := firstNonEmpty(mention.Surface, "entity")
	entityKind := mention.EntityKind
	if entityKind == "" {
		entityKind = schema.EntityKindOther
	}
	rec := schema.NewMemoryRecord(uuid.New().String(), schema.MemoryTypeEntity, schema.SensitivityLow, &schema.EntityPayload{
		Kind:          "entity",
		CanonicalName: canonicalName,
		EntityKind:    entityKind,
		Aliases:       uniqueStrings(append([]string{canonicalName}, mention.Aliases...)),
...
func resolveReferenceCandidate(ref string, candidates []*schema.MemoryRecord) (*schema.MemoryRecord, bool) {
	for _, rec := range candidates {
		if rec == nil {
			continue
		}
		if rec.ID == ref || recordContainsReference(rec, ref) {
			return rec, true
		}
	}
	return nil, false
...
func (s *Service) maybeCreateSemanticRecord(ctx context.Context, sourceRecord *schema.MemoryRecord, req CaptureMemoryRequest, ts time.Time) (*schema.MemoryRecord, []schema.GraphEdge, error) {
	...
	return rec, []schema.GraphEdge{edgeA, edgeB}, nil
}
...
func recordSearchTerms(rec *schema.MemoryRecord) []string {
	...
	case *schema.EntityPayload:
		...
		terms = append(terms, payload.Aliases...)
